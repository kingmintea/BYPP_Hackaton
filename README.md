...
서버는 `0.0.0.0`으로 열려 있어서 **PC와 같은 와이파이에 연결된 휴대폰**에서도 접속할 수 있습니다. `python server.py`로 서버를 켜면 콘솔에 다음처럼 휴대폰용 주소가 함께 출력됩니다.

```
Serving on http://localhost:8420 (자동 확인 스케줄러 동작 중)
같은 와이파이의 휴대폰/다른 기기에서는: http://192.168.x.x:8420/index.html
```

이 주소를 휴대폰 브라우저에 그대로 입력하면 됩니다. 화면 레이아웃도 좁은 화면(약 390px)에 맞게 확인해뒀습니다 — 폼과 버튼은 줄바꿈되고, 게시판 표는 페이지 전체가 아니라 표 안에서만 가로 스크롤됩니다.

접속이 안 되면 확인할 것:
- PC와 휴대폰이 **같은 와이파이**에 연결되어 있는지 (다른 네트워크면 IP로 접속 불가)
- Windows 방화벽이 `python.exe`의 인바운드 연결을 막고 있지 않은지 (`제어판 → Windows Defender 방화벽 → 앱이 방화벽을 통해 통신하도록 허용`에서 Python 허용 확인)
